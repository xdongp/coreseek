//
// $Id: sphinxsearch.cpp 3008 2011-11-13 12:36:32Z klirichek $
//
...
static ISphQword * CreateQueryWord ( const XQKeyword_t & tWord, const ISphQwordSetup & tSetup )
{
	...
	pWord->m_iQueryWeight= tWord.m_iQueryWeight;
	return pWord;
}
	...
	, m_bNotWeighted ( bNotWeighted )
{
	...
	m_iQueryWeight = pQword->m_iQueryWeight;
	m_pHitDoc = NULL;
	m_uHitsOverFor = 0;
...
const ExtHit_t * ExtTerm_c::GetHitsChunk ( const ExtDoc_t * pMatched, SphDocID_t uMaxID )
{
	...
	while ( iHit<MAX_HITS-1 )
	{
		...
		tHit.m_uQueryWeight = (WORD) m_iQueryWeight;
...
struct RankerState_MatchAny_fn : public RankerState_Proximity_fn<false,false>
{
	...
	BYTE m_uQueryWeight[SPH_MAX_FIELDS];
	//int m_iPrint;
	...
	bool Init ( int iFields, const int * pWeights, ExtRanker_c * pRanker, CSphString & sError )
	{
		...
		memset ( m_uQueryWeight, 0, sizeof(m_uQueryWeight) );
		return true;
	}
	...
	DWORD Finalize ( const CSphMatch & )
	{
		...
		for ( int i=0; i<m_iFields; i++ )
		{
			if ( m_uMatchMask[i] ){
				...
				uRank += ( bit + ( m_uQueryWeight[i]*m_iPhraseK*3>>1 ) + ( m_uLCS[i]-1 )*m_iPhraseK )*m_pWeights[i];
