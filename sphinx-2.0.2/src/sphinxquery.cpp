...
static void xqDump ( XQNode_t * pNode, const CSphSchema & tSch, int iIndent, dict *pQueryDict)
{
	if (!pNode) return ;
	if ( pNode->m_dChildren.GetLength() )
	{
		...
		ARRAY_FOREACH ( i, pNode->m_dChildren )
			xqDump ( pNode->m_dChildren[i], tSch, iIndent+1, pQueryDict);
	} else
	{
		xqIndent ( iIndent );
		...
		ARRAY_FOREACH ( i, pNode->m_dWords )
		{
			...
			//int count = dict_get(pQueryDict,(char *)tWord.m_sWord.cstr());
			printf ( " %s (qpos %d%s), QueryWeight: %d ,hehe", tWord.m_sWord.cstr(), tWord.m_iAtomPos, sLocTag, tWord.m_iQueryWeight);
		}
		printf ( "\n" );
	}
}
...
static void xqSetMainWordWeight ( XQNode_t * pNode, int min)
{
	if (!pNode) return ;
	if ( pNode->m_dChildren.GetLength() )
	{
		ARRAY_FOREACH ( i, pNode->m_dChildren )
			xqSetMainWordWeight ( pNode->m_dChildren[i], min);
	} else
	{
		ARRAY_FOREACH ( i, pNode->m_dWords )
		{
			XQKeyword_t & tWord = pNode->m_dWords[i];
			if( tWord.m_iQueryWeight != min)
				tWord.m_iQueryWeight = 0;
			else
				tWord.m_iQueryWeight = 1;
		}
...
static void xqSetWordWeight ( XQNode_t * pNode, dict *pQueryDict, int *min)
{
	if (!pNode) return ;
	if ( pNode->m_dChildren.GetLength() )
	{
		ARRAY_FOREACH ( i, pNode->m_dChildren )
			xqSetWordWeight ( pNode->m_dChildren[i], pQueryDict, min);
	} else
	{
		ARRAY_FOREACH ( i, pNode->m_dWords )
		{
...
bool sphParseExtendedQuery ( XQQuery_t & tParsed, const char * sQuery, const ISphTokenizer * pTokenizer, const CSphSchema * pSchema, CSphDict * pDict, int iStopwordStep )
{
	...
	{
		printf ( "--- query ---\n" );
		xqDump ( tParsed.m_pRoot, *pSchema, 0, pTokenizer->m_pQueryDict);
		printf ( "---\n" );
	}
#endif
